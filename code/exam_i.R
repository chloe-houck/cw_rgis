...
mtcars_g35 <- filter(df_mtcars, gear == 3 | gear == 5)
...
mtcars_light_cyl4 <- filter(df_mtcars, wt < 2.5) %>% 
  filter(cyl == 4)
...
mtcars_ptw_model <- filter(mtcars_with_ptw, cyl == 6) %>% 
  arrange(desc(ptw))
...
mtcars_mpg_by_gear <- group_by(df_mtcars, gear) %>% 
  mutate("min" = min(mpg)) %>% 
  mutate("max" = max(mpg)) %>% 
  ungroup()
